...
- 338 тестов vitest в двух проектах корневого `vitest.config.ts` (node — 273, dom — 65):
...
- Если мутация выживает, дописывается тест, а не удаляется мутация. Таблица ниже получена
  командой `npm run test:mutation -- --markdown`: 236 мутаций, все убиты.
...
| Мутация                                                            | Результат | Какие тесты упали                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| ------------------------------------------------------------------ | --------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| патч вызывает полный рефетч                                        | killed    | patched меняет data записи текущего ключа, фетчер не вызывается, полного расчёта итогов нет; patched не трогает записи других ключей, но помечает их протухшими; пропуск seq вызывает requested({ force }) текущего ключа; патч с пропуском не применяется; hello после переподключения: тот же seq — ничего, другой — перезапрос; повтор уже учтённого seq игнорируется; патч не перемонтирует таблицу и дерево и перерисовывает только строки и карточки узла и предков |
...
| hello после переподключения не сверяет seq                         | killed    | patched меняет data записи текущего ключа, фетчер не вызывается, полного расчёта итогов нет; patched не трогает записи других ключей, но помечает их протухшими; пропуск seq вызывает requested({ force }) текущего ключа; патч с пропуском не применяется; hello после переподключения: тот же seq — ничего, другой — перезапрос; повтор уже учтённого seq игнорируется; патч не перемонтирует таблицу и дерево и перерисовывает только строки и карточки узла и предков |
...
| номера обновлений не записываются                                  | killed    | patched меняет data записи текущего ключа, фетчер не вызывается, полного расчёта итогов нет; патч подсвечивает изменённые ячейки и поля карточек, подсветка гаснет; повторный патч той же ячейки до угасания перезапускает подсветку: новый элемент; prefers-reduced-motion — подсветка без перехода, но не пропадает; значение, обновлённое до появления строки или карточки, не подсвечивается                                                                          |
...
| Мутация                                              | Результат | Какие тесты упали                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| ---------------------------------------------------- | --------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| toggle сбрасывает раскрытия потомков                 | killed    | toggle меняет только сам узел, раскрытия потомков сохраняются; toggle не сбрасывает потомков: свернуть и раскрыть родителя — ветка как была; без пропа expandedIds работает автономно: первый уровень раскрыт, шевроны и кнопки меняют раскрытие; dimUnmatched: несовпавшие приглушены статическим правилом и интерактивны; без него — не приглушены; значение, обновлённое до появления строки или карточки, не подсвечивается; до первого действия раскрытие следует за initialExpandedIds, после — нет |
...
| Мутация                                                           | Результат | Какие тесты упали                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| ----------------------------------------------------------------- | --------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| состояние игнорируется: раскрытие всегда по умолчанию             | killed    | без пропа expandedIds работает автономно: первый уровень раскрыт, шевроны и кнопки меняют раскрытие; dimUnmatched: несовпавшие приглушены статическим правилом и интерактивны; без него — не приглушены; значение, обновлённое до появления строки или карточки, не подсвечивается; до первого действия раскрытие следует за initialExpandedIds, после — нет; методы — операции над раскрытием на экране; expandAncestors — один рендер на всю цепочку; нечего раскрывать — набор тот же |
...
| Мутация                                             | Результат | Какие тесты упали                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| --------------------------------------------------- | --------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| общая численность = собственной                     | killed    | родитель со своими 2 сотрудниками (40) и детьми по 2 (80 и 60): собственные 2 и 40, общие 6 и 60; нет собственных сотрудников — собственная эффективность не определена, общая есть; узел с детьми — пары «собственные / общие»; узел и предки получают seq у изменившихся видимых значений; прочие узлы и значения — нет; удаление и добавление обновляют итоги предков; патч подсвечивает изменённые ячейки и поля карточек, подсветка гаснет; повторный патч той же ячейки до угасания перезапускает подсветку: новый элемент; prefers-reduced-motion — подсветка без перехода, но не пропадает                                                                                                       |
...
| лист показывает все четыре строки                   | killed    | лист — только «Численность» и «Эффективность»; лист без сотрудников — эффективность «—»; значение, обновлённое до появления строки или карточки, не подсвечивается                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| Мутация                                                              | Результат | Какие тесты упали                                                                                                       |
| -------------------------------------------------------------------- | --------- | ----------------------------------------------------------------------------------------------------------------------- |
| стрелки не двигают фокус                                             | killed    | стрелки двигают фокус по строкам; Home/End — к краям; PageDown/PageUp — на экран                                        |
| стрелка вниз не двигает строку                                       | killed    | стрелки двигают фокус по строкам; Home/End — к краям                                                                    |
| Home идёт на строку выше, а не к первой                              | killed    | стрелки двигают фокус по строкам; Home/End — к краям                                                                    |
| End идёт на строку ниже, а не к последней                            | killed    | стрелки двигают фокус по строкам; Home/End — к краям                                                                    |
| фокус привязан к позиции, а не к узлу                                | killed    | фокус сохраняется на узле при смене сортировки; фокус переезжает на соседнюю строку, если узел отфильтрован             |
| отфильтрованный узел: фокус на первую строку, а не на ближайшую      | killed    | фокус переезжает на соседнюю строку, если узел отфильтрован                                                             |
| Enter не выбирает строку                                             | killed    | Enter выбирает строку; Enter работает и после клика мышью                                                                |
| строки, появившиеся после пустой таблицы, забирают фокус             | killed    | пустая таблица теряет фокус; новые строки не забирают                                                                   |
| PageDown и PageUp — на одну строку, а не на экран                    | killed    | PageDown/PageUp — на экран (ResizeObserver)                                                                             |

#### entities-table-row-focus — `packages/entities/org-tree/src/ui/OrgTableRow.tsx`

| Мутация                                     | Результат | Какие тесты упали                                                                                |
| ------------------------------------------- | --------- | ------------------------------------------------------------------------------------------------ |
| все строки в порядке Tab (roving tabindex)  | killed    | Tab входит в таблицу один раз и выходит; стрелки двигают фокус по строкам                        |
| кнопка названия в порядке Tab               | killed    | Tab входит в таблицу один раз и выходит; кнопка названия вне таб-порядка                         |

#### entities-layout-transition — `packages/entities/org-tree/src/model/useLayoutTransition.ts`

| Мутация                                                            | Результат | Какие тесты упали                                                                                                 |
| ------------------------------------------------------------------ | --------- | ----------------------------------------------------------------------------------------------------------------- |
| узел не удаляется после анимации исчезновения                      | killed    | узел удаляется из DOM после анимации исчезновения                                                                 |
| узел удаляется по концу любой анимации внутри него                 | killed    | узел удаляется из DOM после анимации исчезновения; анимация появления не удаляет узел                              |
| пропавший узел удаляется сразу, без исчезновения                   | killed    | узел анимирует исчезновение (opacity 0)                                                                           |
| новый узел появляется без анимации                                 | killed    | узел анимирует появление (opacity, transform)                                                                     |
| узел появляется на своём месте, а не из родителя                   | killed    | узел выезжает из позиции родителя при появлении                                                                   |
| вернувшийся до конца исчезновения узел остаётся исчезающим          | killed    | узел, вернувшийся до конца анимации исчезновения, появляется снова                                                |
| первые данные появляются с анимацией                               | killed    | первая загрузка без анимации появления (нет прежней раскладки)                                                    |

#### entities-tree-motion — `packages/entities/org-tree/src/ui/TreeCanvas.tsx`

| Мутация                                                  | Результат | Какие тесты упали                                                                                        |
| -------------------------------------------------------- | --------- | -------------------------------------------------------------------------------------------------------- |
| reduced-motion игнорируется                              | killed    | prefers-reduced-motion — анимации нет (длительность 0)                                                    |
| при reduced-motion длительность не обнуляется            | killed    | prefers-reduced-motion — анимации нет (длительность 0)                                                    |
| конец анимации не слушается: исчезнувший узел остаётся   | killed    | узел удаляется из DOM после анимации исчезновения                                                        |
| исчезающий узел считается узлом раскладки                 | killed    | исчезающий узел не ловит клики (data-node-id снят)                                                        |
| исчезающий узел доступен скринридеру                     | killed    | исчезающий узел скрыт от скринридера (aria-hidden)                                                        |

#### entities-tree-edge-motion — `packages/entities/org-tree/src/ui/TreeEdge.tsx`

| Мутация                                              | Результат | Какие тесты упали                                           |
| ---------------------------------------------------- | --------- | ----------------------------------------------------------- |
| рёбра появляются и исчезают без анимации              | killed    | рёбра анимируются при появлении и исчезновении (data-motion) |
...
| Решения    | Node 24; остановка dev-сервера; журнал отдельными файлами; `isEqual` по `id → updatedAt`; страница-заглушка; отказ от клавиатурной навигации по дереву стрелками; приложение без StrictMode; мутационные скрипты в репозитории; структура «слой — группа пакетов, пакет — функциональность»; `level` таблицы — глубина дерева на клиенте; двойной клик по заголовку — то же переключение направления, без `onDoubleClick`; фильтр, сортировка и режим в адресе (replace / push), режимы и схлопывание split без переписывания адреса, координация на странице, правило «модель в хуке» для `CLAUDE.md`; группировка строк таблицы без сворачивания | всё остальное: устройство пакетов внутри этой структуры, реализации, выбор библиотек в рамках заданного стека, тесты и мутации, проверки в браузере |
