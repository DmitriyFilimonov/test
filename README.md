...
`make up` удалит старые контейнеры, соберёт образы и запустит сервисы. Работает и с Compose v2
(`docker compose`), и с v1 (`docker-compose`) — определяется автоматически. `.env` не обязателен:
без него действуют умолчания, генерация патчей стартует сама. Подробности —
...
Команда `make up` удаляет старые контейнеры перед запуском, чтобы избежать проблем с устаревшими
образами (например, `KeyError: 'ContainerConfig'` в docker-compose 1.29.x).
